...
class ChatSession:
    ...
    async def start(self) -> None:
        ...
        tool_descriptions = "\n".join(
            f"- {tool.name}: {tool.description}\n  Required args: {', '.join(tool.inputSchema.get('properties', {}).keys()) if tool.inputSchema else 'None'}"
        ...
        messages = [{
            "role": "system",
             "content": (
                ...
                "If a tool is needed, respond with ONLY a valid JSON:\n"
            ...
            if llm_response.strip().startswith("{"):
                try:
                    ...
                    for server in self.servers:
                        ...
                        print("⚠️ Tool not found in any server.")
                except Exception as e:
                    print(f"❌ Error parsing or executing tool: {e}")
            else:
                print(f"💬 OpenAI Response: {llm_response}")

async def main():
    config = Configuration()
    servers_config = config.load_config("my_app/servers_config.json")
